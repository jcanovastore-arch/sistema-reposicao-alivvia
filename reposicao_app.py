    ...
    if st.session_state.catalogo_df is not None:
        ...
        def processar(emp):
            ...
            try:
                ...
                # Lógica: O data.py JÁ LIMPOU OS NOMES (ex: estoque_atual). 
                # O logic.py corrigido espera exatamente isso.
        ...
        for emp in ["ALIVVIA", "JCA"]:
            ...
            if res is not None:
                ...
                m1.metric("Total Físico", f"{tot_fis:,}".replace(",", "."))
                m2.metric("Total Full (Estimado)", f"{tot_full:,}".replace(",", "."))
...
with tab3: st.info("Editor de OC (Em desenvolvimento)")
with tab4: st.info("Gestão (Em desenvolvimento)")
with tab5: st.info("Alocação (Em desenvolvimento)")